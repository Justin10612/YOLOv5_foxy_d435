...
class yolov5_ros(Node):
    ...
    def __init__(self):
        ...
        # 
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/sss0301/ros2_ws/src/detection/models/bestv8.pt')
    ...
    def dectshow(self, org_img, boxs, depth_data):
        ...
        def get_mid_pos(box, depth_data):
            ...
            depth_heigh = 300
            bias = 10
            target_x = (box[0] + box[2])//2    # The center-x of the bounding box
            # Get the smaple point
            for i in range(3):
                distance_list.append(depth_data[int(target_x), int(depth_heigh+bias)])
                bias +=10
            # Let you know where the smaple point is.
            cv2.circle(org_img, (int(target_x), int(depth_heigh)), 8, (255,255,255), 2)
            distance_list.append(depth_data[int(target_x+bias), int(depth_heigh)])
            return [target_x, np.mean(distance_list)]
        ...
        for box in boxs:
            ...
            cv2.putText(org_img, "Target" + str(float(pos[1]) / 1000)[:4] + 'm',
